...
from src.agent.tool_recorder import (
    ToolRecorder,
    get_tool_recorder,
    record_tool_call,
)
...
def test_tool_recorder():
    ...
    for tool, tool_stats in stats['tool_stats'].items():
        print(f"  {tool}: {tool_stats['count']}次, 成功率 {tool_stats['success_rate']:.1f}%")
    
    # 查询功能
    print("\n--- 查询示例 ---")
    print(f"按会话查询 (sess_001): {len(recorder.get_calls_by_session('sess_001'))} 条")
    print(f"按工具查询 (weather_query): {len(recorder.get_calls_by_tool('weather_query'))} 条")
