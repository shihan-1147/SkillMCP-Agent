...
async def test_rag_pipeline():
    ...
    # 导入 RAG 组件
    from src.rag import (ChunkStrategy, DocumentLoader, RAGPipeline,
                         TextChunker, get_rag_pipeline)
    ...
    chunker = TextChunker(chunk_size=300, overlap=30, strategy=ChunkStrategy.MARKDOWN)
    ...
    rag = RAGPipeline(
        embedder=MockEmbedder(dimension=384), chunk_size=300, chunk_overlap=30
    )
    ...
    if docs_dir.exists():
        ...
        await rag.add_document(
            content="AI Agent 是能够自主执行任务的智能系统。", title="AI Agent 简介"
        )
    ...
    print("-" * 40)
    print(
        augmented_prompt[:800] + "..."
        if len(augmented_prompt) > 800
...
async def test_agent_with_rag():
    ...
    if docs_dir.exists():
        ...
        await rag.add_document(
            content="AI Agent 包含规划器、执行器、记忆系统等组件。", title="Agent 组件"
        )
    ...
    result = await rag_tool.execute(query="AI Agent 有哪些核心组件?", top_k=3)
