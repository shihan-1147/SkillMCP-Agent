...
async def test_tools():
    ...
    # 导入工具
    from src.mcp.tools import (RAGRetrieverTool, SystemTimeTool,
                               TrainQueryTool, WeatherQueryTool)
    ...
    if result["success"]:
        ...
        print(
            f"空气质量: {data['air_quality']['level']} (AQI: {data['air_quality']['aqi']})"
        )
    ...
    if result["success"]:
        ...
        for forecast in result["data"]["forecasts"][:3]:
            print(
                f"  {forecast['date']} ({forecast['weekday']}): "
    ...
    result = await train_tool.execute(
        action="get_station_code", station_names="北京,上海,广州"
    )
    print(f"站点代码: {result['data']}")

    # 查询余票
    result = await train_tool.execute(
        action="query_tickets", origin="北京", destination="上海", date="2025-01-20"
    )
    if result["success"]:
        print(
    ...
    from src.mcp import call_tool, initialize_mcp
