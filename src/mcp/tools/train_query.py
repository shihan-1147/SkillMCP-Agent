...
class TrainQueryTool(BaseTool):
    ...
    async def execute(self, **kwargs) -> Dict[str, Any]:
        ...
        if action == "get_current_date":
            ...
            if not origin or not destination:
                return {"success": False, "error": "缺少出发站或到达站参数"}

            return await self._query_tickets_via_mcp(
                origin, destination, date, train_type
            )
    ...
    async def _get_station_codes_via_mcp(self, city_names: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result = await mcp.call_tool(
                "12306-mcp", "get-station-code-of-citys", {"citys": city_names}
            )
    ...
    async def _query_tickets_via_mcp(
        self, origin: str, destination: str, date: str = None, train_type: str = "GD"
    ) -> Dict[str, Any]:
        """
        通过 MCP 查询火车票
        ...
        try:
            ...
            if not origin_code:
                code_result = await mcp.call_tool(
                    "12306-mcp", "get-station-code-of-citys", {"citys": origin}
                )
            ...
            if not dest_code:
                code_result = await mcp.call_tool(
                    "12306-mcp", "get-station-code-of-citys", {"citys": destination}
                )
                if code_result.get("success"):
                    try:
                        data = code_result.get("data", "")
            ...
            # 调用 12306-mcp 查询余票
            logger.info(
                f"Calling 12306-mcp: {origin}({origin_code}) -> {destination}({dest_code}) on {date}"
            )
    ...
    def _parse_mcp_train_data(self, raw_data: str) -> List[Dict[str, Any]]:
        ...
        for line in raw_data.strip().split("\n"):
            ...
            # 匹配座位行
                ...
                seat_match = re.match(
                    r"^-\s*(.+?):\s*(剩余(\d+)张票|有票|无票)\s*(\d+)元?", line
                )
                if seat_match:
