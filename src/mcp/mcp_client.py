...
class MCPClientManager:
    ...
    async def initialize(self):
        ...
        logger.info(f"MCP Client Manager initialized, {len(self._sessions)} servers connected")
    ...
    async def _connect_stdio_server(self, name: str, config: Dict):
        ...
        logger.info(f"Connecting to MCP server {name} via stdio: {command} {' '.join(args)}")
        ...
        try:
            ...
            logger.info(f"Connected to MCP server {name}, available tools: {[t['name'] for t in tools]}")
