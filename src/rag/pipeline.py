...
class RAGPipeline:
    ...
    def __init__(
        ...
        self.chunker = TextChunker(
            chunk_size=chunk_size, overlap=chunk_overlap, strategy=chunk_strategy
        )
        ...
        self.retriever = Retriever(
            ...
            config=RetrievalConfig(top_k=settings.rag_top_k, score_threshold=0.3),
        )
    ...
    async def load_documents(
        self, source: Union[str, Path, List[str]], recursive: bool = True
    ) -> int:
        """
        加载并索引文档
        ...
        if isinstance(source, (str, Path)):
            ...
            for src in source:
                ...
                if path.is_file():
                    ...
                    documents.extend(
                        self.loader.load_directory(path, recursive=recursive)
                    )
    ...
    async def retrieve(
        self, query: str, top_k: int = None, filters: Dict[str, Any] = None
    ...
    def get_context_for_prompt(
        self, results: List[RetrievalResult], max_length: int = 4000
    ) -> str:
        """
        获取用于 Prompt 的上下文
    ...
    def build_augmented_prompt(
        self, query: str, results: List[RetrievalResult], system_prompt: str = None
    ) -> str:
        """
        构建增强 Prompt
