...
class Settings(BaseSettings):
    ...
    ollama_model: str = Field(default="gemini-3-flash-preview:latest", description="Ollama 模型")
    ...
    # Embedding 配置 - Ollama
    embedding_provider: str = Field(default="ollama", description="Embedding 提供者: ollama/openai/local/mock")
    ollama_embedding_model: str = Field(default="qwen3-embedding:latest", description="Ollama Embedding 模型")
    embedding_model: str = Field(default="text-embedding-3-small", description="OpenAI Embedding 模型")
    ...
    rag_embedder_type: str = Field(default="ollama", description="Embedder 类型: ollama/openai/local/mock")
    rag_index_path: str = Field(default="data/rag_index", description="RAG 索引存储路径")
