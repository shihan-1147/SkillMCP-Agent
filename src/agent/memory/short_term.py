...
class ShortTermMemory:
    ...
    def __init__(self, max_messages: int = None):
        ...
        logger.debug(
            f"ShortTermMemory initialized with max_messages={self.max_messages}"
        )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            "system_message": (
                self._system_message.content if self._system_message else None
            ),
            "messages": [
                {"role": m.role, "content": m.content[:100]} for m in self._messages
            ],
