...
class ChatService:
    ...
    async def chat(
        ...
        logger.info(f"Chat request: session={session.session_id}, message={request.message[:50]}...")
        ...
        try:
            ...
            if debug and debug_info:
                ...
                debug_info["tool_calls"] = [
                    t.to_dict() for t in tracer.tool_calls
                ]
            ...
            return ChatResponse(
                ...
                reply=error_reply,
                session_id=session.session_id,
                debug_info={"error": str(e), "trace": tracer.get_timeline()} if debug else None,
            )
    ...
    async def _process_message(
    ...
    ) -> Tuple[str, Optional[List[StructuredData]], Optional[List[str]], Optional[Dict]]:
        """
        处理用户消息
        ...
        if intent == "weather":
            ...
            reply, data = await self._handle_weather(message, tracer, tool_recorder, session.session_id)
            if data:
            ...
            reply, data = await self._handle_train(message, tracer, tool_recorder, session.session_id)
            if data:
                structured_data.append(data)
            ...
            reply, data, rag_sources = await self._handle_knowledge(message, tracer, tool_recorder, session.session_id)
            if data:
                structured_data.append(data)
    ...
    async def _handle_weather(
        ...
        try:
            if self._mcp:
                ...
                if tracer:
                    tracer.log_event(TraceEventType.MCP_CALL_START, {"tool": "weather_query", "city": city})
    ...
    async def _handle_train(
        ...
        try:
            if self._mcp:
                ...
                duration_ms = (time.time() - start_time) * 1000
                if tracer:
                    tracer.log_tool_call("system_time", {"action": "get_current"}, date_result, True, duration_ms=duration_ms)
                ...
                if "明天" in message:
                    ...
                    duration_ms = (time.time() - start_time) * 1000
                    if tracer:
                        tracer.log_tool_call("system_time", {"action": "parse_relative"}, date_result, True, duration_ms=duration_ms)
                ...
                if tracer:
                    tracer.log_tool_call(
                        ...
                        arguments={"origin": origin, "destination": destination, "date": today},
                        result=result,
                        success=result.get("success", False),
                        duration_ms=duration_ms
                    )
    ...
    async def _handle_knowledge(
        ...
        try:
            if self._rag and self._rag._initialized:
                ...
                start_time = time.time()
                if tracer:
                    tracer.log_event(TraceEventType.RAG_QUERY_START, {"query": message[:50]})
                ...
                if results:
                    ...
                    # 收集来源
                    sources = list(set(
                        r.chunk.metadata.get("title", "unknown")
                        for r in results
                    ))
            ...
            if self._mcp:
                start_time = time.time()
                if tracer:
                    tracer.log_event(TraceEventType.MCP_CALL_START, {"tool": "rag_retriever"})
    ...
    def _extract_route(self, message: str) -> Tuple[Optional[str], Optional[str]]:
        ...
        # 先移除时间相关的词
        clean_message = re.sub(r'(今天|明天|后天|大后天|下周[一二三四五六日天]?|这周[一二三四五六日天]?|周[一二三四五六日天])', '', message)
