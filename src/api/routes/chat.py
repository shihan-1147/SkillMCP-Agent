...
from src.api.deps import (get_chat_service_dep, get_debug_mode,
                          get_session_manager_dep)
from src.api.schemas import (ChatRequest, ChatResponse, ResponseStatus,
                             SessionInfo)
...
@router.post("/stream", summary="流式对话", description="发送消息并获取流式响应")
    ...
    async def generate():
        try:
            ...
            for i, char in enumerate(response.reply):
                chunk = {"event": "token", "data": char, "index": i}
...
async def get_session(
    session_id: str, session_manager: SessionManager = Depends(get_session_manager_dep)
) -> SessionInfo:
    """
    获取会话信息
...
@router.delete(
    "/session/{session_id}", summary="删除会话", description="删除指定会话及其历史记录"
)
...
@router.get("/sessions", summary="列出所有会话", description="获取所有活跃会话的列表")
