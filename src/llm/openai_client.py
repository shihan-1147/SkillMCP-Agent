...
class OpenAIClient(BaseLLM):
    ...
    def __init__(
        self, api_key: str = None, base_url: str = None, model: str = None, **kwargs
    ):
        """
        初始化 OpenAI 客户端
    ...
    async def chat(
        ...
        **kwargs,
        ...
        try:
            ...
            raise LLMException(
                message=f"LLM 调用失败: {str(e)}", details={"model": self.model}
            )
    ...
    async def stream_chat(
        self, messages: List[Dict[str, Any]], temperature: float = 0.7, **kwargs
    ) -> AsyncIterator[str]:
        """
        流式聊天
        ...
        try:
            ...
            raise LLMException(
                message=f"LLM 流式调用失败: {str(e)}", details={"model": self.model}
            )
