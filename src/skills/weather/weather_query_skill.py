...
class WeatherQuerySkill(BaseSkill):
        ...
        **kwargs,
        ...
        try:
            ...
            weather_data = await mcp_client.call_tool(
                "mcp_amap_amap_maps_weather", {"city": city}
            )
    ...
    def _format_result(self, city: str, weather_data: Any) -> Dict[str, Any]:
        """格式化天气查询结果"""
        result = {
            "success": True,
            "data": {
        ...
        if isinstance(weather_data, dict):
            ...
            result["data"]["wind"] = (
                f"{wind} {wind_power}级" if wind and wind_power else wind
            )
            result["data"]["update_time"] = weather_data.get("reporttime")

            # 生成建议
            result["data"]["suggestion"] = self._generate_suggestion(
                weather, temperature
            )
    ...
    def _generate_suggestion(
        self, weather: Optional[str], temperature: Optional[str]
    ) -> str:
        """根据天气生成建议"""
        if not weather:
