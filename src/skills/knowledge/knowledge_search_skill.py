...
class KnowledgeSearchSkill(BaseSkill):
    ...
    async def execute(
        ...
        **kwargs,
        ...
        try:
            ...
            # 方式1: 使用内置检索器
            if self._retriever:
                results = await self._search_with_retriever(query, top_k, threshold)
            # 方式2: 通过 MCP 调用
            elif mcp_client:
                ...
                return {"success": False, "error": "未配置检索器或 MCP 客户端"}
    ...
    def _parse_params(
        self, description: str, tool_params: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        """解析查询参数"""
    ...
    async def _search_with_retriever(
        ...
        """使用内置检索器搜索"""
        # 调用 RAG 检索器
        raw_results = await self._retriever.retrieve(query=query, top_k=top_k)
    ...
    async def _search_via_mcp(
        self, mcp_client: "MCPClient", query: str, top_k: int, threshold: float
    ) -> List[Dict[str, Any]]:
        """通过 MCP 调用 RAG 工具"""
        result = await mcp_client.call_tool(
            "rag_retriever", {"query": query, "top_k": top_k, "threshold": threshold}
        )
    ...
    def build_rag_prompt(
        self, query: str, context: str, instruction: str = None
    ) -> str:
        """
        构建 RAG 增强的 Prompt
