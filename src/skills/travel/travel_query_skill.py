...
class TravelQuerySkill(BaseSkill):
    ...
    name = "travel_query"
    description = (
        "查询火车、高铁、动车票务信息，包括车次、余票、时刻表等。适用于出行规划场景。"
    )
    required_tools = [
    ...
    async def execute(
        ...
        **kwargs,
        ...
        try:
            ...
            # 步骤3: 获取站点代码
            origin_code = await self._get_station_code(mcp_client, params["origin"])
            dest_code = await self._get_station_code(mcp_client, params["destination"])
            ...
            tickets = await self._query_tickets(
                mcp_client, origin_code, dest_code, travel_date
            )
            ...
            logger.info(
                f"Travel query completed: found {len(result.get('trains', []))} trains"
            )
            return result
    ...
    async def _get_current_date(self, mcp_client: "MCPClient") -> str:
        ...
        try:
            result = await mcp_client.call_tool("mcp_12306-mcp_get-current-date", {})
    ...
    async def _get_station_code(
        self, mcp_client: "MCPClient", station_name: str
    ) -> Optional[str]:
        """获取站点代码"""
        try:
            result = await mcp_client.call_tool(
                "mcp_12306-mcp_get-station-code-by-names", {"names": station_name}
            )
            # 解析结果获取代码
    ...
    async def _query_tickets(
        self, mcp_client: "MCPClient", origin_code: str, dest_code: str, date: str
    ) -> Any:
        """查询余票"""
        return await mcp_client.call_tool(
            "mcp_12306-mcp_get-tickets",
            {"origin": origin_code, "destination": dest_code, "date": date},
        )
    ...
    def _format_result(
        ...
        if isinstance(tickets, list):
            for ticket in tickets:
                ...
                if train_info:
                    # 车型筛选
                    if train_type and not train_info.get("train_no", "").startswith(
                        train_type
                    ):
                        continue
